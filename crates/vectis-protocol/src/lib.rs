...
pub enum ProtocolMessage {
    ClientHello { room: String, state_vector: Vec<u8> },
    ServerWelcome { actor: ActorId, color: u32 },
    Snapshot { bytes: Vec<u8> },
    Update { bytes: Vec<u8> },
    StateVector { bytes: Vec<u8> },
...
pub fn encode_message(message: &ProtocolMessage) -> Vec<u8> {
    ...
    match message {
        ProtocolMessage::ClientHello { room, state_vector } => {
            out.push(TAG_CLIENT_HELLO);
            encode_string(room, &mut out);
            encode_bytes(state_vector, &mut out);
        }
        ProtocolMessage::ServerWelcome { actor, color } => {
