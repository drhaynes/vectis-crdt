...
14. [Client/server demo architecture](#14-clientserver-demo-architecture)
...
### `ActorId(u64)` — `crates/vectis-crdt/src/types.rs`
...
### `LamportTs(u64)` — `crates/vectis-crdt/src/types.rs`
...
### `OpId { lamport: LamportTs, actor: ActorId }` — `crates/vectis-crdt/src/types.rs`
...
### `VectorClock` — `crates/vectis-crdt/src/types.rs`
...
`RgaArray` in `crates/vectis-crdt/src/rga.rs` is the sequence structure that maintains the z-order of strokes.
...
`RgaArray::integrate` in `crates/vectis-crdt/src/rga.rs:86`:
...
`crates/vectis-crdt/src/causal_buffer.rs`
...
### `Document` — `crates/vectis-crdt/src/document.rs`
...
### `LwwRegister<T>` — `crates/vectis-crdt/src/stroke.rs`
...
### `LwwMap<K, V>` — `crates/vectis-crdt/src/document.rs`
...
### `Aabb` — `crates/vectis-crdt/src/stroke.rs`
...
## 14. Client/server demo architecture

### Compilation

The core `vectis-crdt` crate is a pure Rust library under `crates/vectis-crdt`. Shared WebSocket message framing lives in `crates/vectis-protocol`, demo client behavior lives in `crates/app-core`, the browser host lives in `crates/wasm_demo`, and the in-memory WebSocket room server lives in `crates/vectis-server`:
...
- startup via `#[wasm_bindgen(start)]`
- DOM lookup and control event binding
- pointer events from the canvas
- WebSocket binary frames
- `requestAnimationFrame`
- `CanvasRenderingContext2d` drawing

CRDT state is not mediated through a JavaScript-facing document wrapper. `app-core` and `vectis-server` use the same Rust API as native callers:
...
Rendering is host-driven. `app-core` exposes platform-neutral stroke views; `wasm_demo` renders those views with browser Canvas2D. A future native demo can render the same views with a native renderer.
...
This removes the old JS render-buffer parsing path. The only JavaScript in `crates/wasm_demo/index.html` is the module loader generated by `wasm-pack` plus a tiny `init()` call. Collaboration is demonstrated by opening the same room URL in multiple tabs.
...
### Property tests (7) — `crates/vectis-crdt/tests/convergence_props.rs`
